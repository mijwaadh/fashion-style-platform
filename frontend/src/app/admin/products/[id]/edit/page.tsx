...
function EditProductContent() {
    ...
    useEffect(() => {
        ...
        api.get<any>(`/api/products/${id}`)
            .then(p => {
                ...
                setMainCategory(p.mainCategory || 'MEN FASHION');
                ...
                if (p.specifications) {
                    setWeightGms(String(p.specifications.weight_gms || ''));
                    setSupplierId(p.specifications.supplier_id || '');
                    setFabric(p.specifications.fabric || '');
                    setFit(p.specifications.fit || '');
                    setNeck(p.specifications.neck || '');
                    setOccasion(p.specifications.occasion || '');
                    setPattern(p.specifications.pattern || '');
                    setSleeveLength(p.specifications.sleeve_length || '');
                    setCountryOfOrigin(p.specifications.country_of_origin || 'India');
                    setManufacturerName(p.specifications.manufacturer_name || '');
                    setManufacturerAddress(p.specifications.manufacturer_address || '');
                    setManufacturerPincode(String(p.specifications.manufacturer_pincode || ''));
                    setPackerName(p.specifications.packer_name || '');
                    setPackerAddress(p.specifications.packer_address || '');
                    setPackerPincode(String(p.specifications.packer_pincode || ''));
                    setImporterName(p.specifications.importer_name || '');
                    setImporterAddress(p.specifications.importer_address || '');
                    setImporterPincode(String(p.specifications.importer_pincode || ''));
                    setSellerComment(p.specifications.seller_comment || '');
                }
    ...
    const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (e.target.files) {
            ...
            setImageFiles(prev => [...prev, ...validFiles].slice(0, 5 - existingImages.length));
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!name || !brand || !price || !mainCategory || !category || !subCategory || !productType || (existingImages.length === 0 && imageFiles.length === 0)) {
            setError('Please fill in all required fields and ensure at least one image is present.');
            return;
        }
                ...
                <div className="flex items-center gap-3 mb-8">
                    ...
                    <div>
                        <div className="text-sm text-muted-foreground mb-0.5">
                            ...
                            <span className="mx-1">›</span> Edit Product
                        </div>
                        <h1 className="font-serif text-3xl font-bold text-foreground flex items-center gap-2">
                            <Package className="w-7 h-7 text-primary" /> Edit Product Details
                        </h1>
                    </div>
                </div>
                        ...
                        <div className="bg-background rounded-3xl p-6 border border-border shadow-sm sticky top-24">
                            ...
                            <div className="grid grid-cols-1 gap-4">
                                {existingImages.map((img, idx) => (
                                    <div key={`existing-${idx}`} className="relative aspect-square rounded-2xl border-2 border-border overflow-hidden bg-muted group">
                                        <Image src={img} alt="Existing" fill className="object-cover" />
                                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                            <Button type="button" variant="destructive" size="sm" className="rounded-full font-bold" onClick={() => removeExistingImage(idx)}>
                                                Remove
                                            </Button>
                                        </div>
                                        {idx === 0 && <div className="absolute top-2 left-2 px-2 py-0.5 bg-zinc-900/80 text-white text-[10px] font-bold rounded-md backdrop-blur-sm">MAIN</div>}
                                    </div>
                                ))}
                                
                                {imagePreviews.map((preview, idx) => (
                                    <div key={`new-${idx}`} className="relative aspect-square rounded-2xl border-2 border-dashed border-primary/30 overflow-hidden bg-muted group">
                                        <Image src={preview} alt="New Preview" fill className="object-cover" />
                                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                            <Button type="button" variant="destructive" size="sm" className="rounded-full font-bold" onClick={() => removeNewImage(idx)}>
                                                Remove
                                            </Button>
                                        </div>
                                    </div>
                                ...
                                {(existingImages.length + imagePreviews.length) < 5 && (
                                    <div className="relative aspect-square w-full rounded-2xl border-2 border-dashed border-border hover:border-primary transition-all bg-muted/30 overflow-hidden group cursor-pointer">
                                        <div className="absolute inset-0 flex flex-col items-center justify-center text-muted-foreground p-6 text-center">
                                            ...
                                            <p className="text-sm font-bold text-foreground">Add Image</p>
                                        </div>
                                        <input type="file" accept="image/*" multiple onChange={handleImageChange} className="absolute inset-0 opacity-0 cursor-pointer" />
                                    </div>
                                )}
                            </div>
                            <p className="text-[10px] text-muted-foreground mt-4 italic text-center">Max 5 images total.</p>
                        </div>
                    </div>
