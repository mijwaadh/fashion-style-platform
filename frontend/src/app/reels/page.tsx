...
function VideoReel({ look, isActive }: { look: Look; isActive: boolean }) {
    ...
    return (
        <div className="relative w-full h-full bg-black flex justify-center overflow-hidden">
            {/* Media Player: Video or Image Carousel */}
            {look.videoUrl ? (
                <video
                    ref={videoRef}
                    src={look.videoUrl}
                ...
                <div className="w-full h-full lg:max-w-md lg:rounded-xl lg:my-auto lg:h-[90%] flex overflow-x-auto snap-x snap-mandatory no-scrollbar relative">
                    {look.images.map((img, idx) => (
                        <div key={idx} className="w-full h-full shrink-0 snap-start snap-always relative">
                            <Image src={img} alt={`${look.title} - ${idx + 1}`} fill className="object-cover" />
                        </div>
                    ))}
                    {look.images.length > 1 && (
                        <div className="absolute top-4 right-4 bg-black/60 backdrop-blur text-white text-[10px] font-bold px-3 py-1.5 rounded-full z-10 pointer-events-none">
                            Swipe &rarr;
                        </div>
                    )}
                </div>
            ) : (
                <div className="w-full h-full lg:max-w-md lg:rounded-xl lg:my-auto lg:h-[90%] relative">
                    <Image src={look.imageUrl} alt={look.title} fill className="object-cover" />
                </div>
            )}
    ...
    useEffect(() => {
        const fetchReels = async () => {
            try {
                ...
                const videoLooks = (data?.looks || []).filter((l: Look) => !!l.videoUrl || (l.images && l.images.length > 0));
                setLooks(videoLooks);
            } catch (error) {
        ...
        <div className="h-screen flex flex-col bg-black">
            ...
            {loading ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-white/50 space-y-4">
                    ...
                    <Link href="/" className="text-primary hover:underline">Go to Discover</Link>
                </div>
            ) : (
                <div 
